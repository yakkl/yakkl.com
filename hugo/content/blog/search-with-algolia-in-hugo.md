...
---
For this week on [_Frontend Friday_](https://yakkl.com/categories/frontend-friday/ "frontend friday tag")_,_ we'll be covering how to set up lightning ⚡️ fast search for your Hugo site using [Algolia](https://algolia.com), the SaaS (Search as a Service 😉 ) provider. We published a Jekyll-focused version of this guide [last week](https://yakkl.com/blog/search-with-algolia-in-jekyll).

Algolia's self-proclaimed claim-to-fame is that they are_"the most reliable platform for building search into your business,"_ and honestly, it's hard to disagree. Yakkl's search is powered by Algolia _(just try searching for Algolia in the search above!)._

![](/uploads/2018/02/yakkl-algolia-search.gif)
...
The path to the index file in the [_Hugo Boilerplate_](https://github.com/yakkl-templates/hugo-boilerplate) is `dist/algolia.json`, whereas the path in a default hugo site is `public/algolia.json`
...
Having to run the NPM script manually each time your site changes isn't ideal, especially when using services like the [Yakkl.com CMS](https://yakkl.com).

That's why we've created an [open-source template](https://github.com/yakkl-templates/serverless-atomic-algolia) for creating a Serverless [Webtask Function](https://webtask.io/) that can automatically update your Algolia index each time your site is updated using web hooks.
...
To get started, clone the [template repository](https://github.com/yakkl-templates/serverless-atomic-algolia) to your local machine by running:

    git clone https://github.com/yakkl-templates/serverless-atomic-algolia.git

{{% tip %}}

Don't have or want to use Git? Feel free to [download the template](https://github.com/yakkl-templates/serverless-atomic-algolia/archive/master.zip) as a zip instead.
...
Head over to the _Settings_ page of your site in Yakkl, and scroll down to the _Webhook URL_ setting.

Enter the URL you received when deploying your function, and then click _Save Settings_.

Now each time Yakkl finishes deploying your site, your function will be invoked to update your Algolia index.

{{% tip %}}

This set up only works when Yakkl is set up to handle the build and deployment of your site. If you're using a third party CI service to build your site (like GitLab CI or Netlify), you will need to use their webhook features to trigger your function.
...
<p><strong>Next week:</strong>  <a href="https://yakkl.com/blog/up-and-running-with-hugo/">Up & Running with Hugo: Building Your First Site</a></p>  
<p><strong>Last week:</strong> We wrote a Jekyll-focused version of this article: <a href="https://yakkl.com/blog/search-with-algolia-in-hugo/">Jekyll Search with Algolia and Webtasks </a>.</p>  
</div>
