...
This blog post will show you how easy it is to integrate Snipcart with a Hugo website. We will also use Yakkl to complete the experience with a product management UI.

## The Demo Project

Accompanying this blog post is a sample Hugo website that integrates Snipcart to provide an e-commerce frontend. [Try out the live demo](https://dwalkr.github.io/snipcart-hugo-demo/) to get a feel for how Snipcart works, or [take a look at the source code](https://github.com/dwalkr/snipcart-hugo-demo) to see how it was done. If you want to get started right away, you can import this demo site into Yakkl with the button below. Be sure to [set up your API key](#getting-started-with-snipcart) after importing the site to enable the Snipcart features.
...
We're using [Create Static Site](https://github.com/yakkl/create-static-site) to set up a new Hugo project with a production-ready build pipeline. We can invoke `create-static-site` with `npx`:
...
{{% tip %}}
Check out our blog post on [getting started with Create Static Site](https://yakkl.com/blog/instant-production-ready-scaffolding-with-create-static-site/) for more information on using this utility.
{{% /tip %}}
...
{{% tip %}}
If you've already imported the demo to Yakkl, you will want to log in to the site's dashboard and navigate to the **Site Params** section to add your API key.
<br /><br />
<a href="https://app.yakkl.com/quick-start?repo=dwalkr/snipcart-hugo-demo&engine=hugo&config=site" class="create-site-button" data-proofer-ignore>Import Our Snipcart Demo</a>
{{% /tip %}}
...
In order to assist our users with product creation, we could create an [archetype](https://gohugo.io/content-management/archetypes/) for products that will initialize these special front matter fields when we create a product with the `hugo new` command. Since we're using Yakkl, however, we will instead create a Front Matter Template in Yakkl that will configure the UI with the appropriate fields whenever someone creates a new product.
...
We're going to use Yakkl's [Blocks](https://yakkl.com/blog/blocks-give-your-editors-the-power-to-build-pages/) feature to define some custom field types. This will enable us to specify any number of custom fields for each individual product.
...
All that's left is to implement these options in our template. We're going to iterate over each option and include a different partial depending on what Front Matter Template the option is using. If you've read [our blog post about Blocks and the Sawmill demo theme](https://yakkl.com/blog/sawmill-layout-composer-for-hugo-and-yakkl), this ["block loop" pattern](https://yakkl.com/blog/sawmill-layout-composer-for-hugo-and-yakkl/#hugo-example) will look familiar to you. The new HTML for our Snipcart button is as follows:
...
Flip the environment toggle in your Snipcart dashboard over to "Live" to access your production settings and API key. When you're ready to go live, you just have to generate a live API key and enter it in the `snipcart_live_api_key` field in the `site/config.toml` file, or in your **Site Params** in Yakkl.
...
<div style="margin-top: 2em; padding: 20px 40px;background: #f7f7f7;"><h2>Join us every Friday :date:</h2><p><a href="/categories/frontend-friday/">Frontend Friday</a> is a weekly series where we write in-depth posts about modern web development.</p><p><strong>Next week:</strong> We'll dig deeper into <a href="https://yakkl.com/blog/hugo-json-api-part-2/">creating a JSON API with Hugo.</a> </p><p><strong>Last week:</strong> We told you about <a href="https://yakkl.com/blog/what-s-new-in-jekyll-3.8/">the latest updates to Jekyll</a>.</p></div>
