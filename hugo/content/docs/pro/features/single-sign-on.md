...
`Username`, `Name`, and `MemberOf` are not used by Yakkl but may still be sent.

The NameID format used by Yakkl is `urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress`.
...
After the provider has been created within Yakkl, fields in the `Service Provider Info` section will need to be added to your SAML provider settings.
...
If only one SAML provider is configured within Yakkl, it will immediately authenticate through SSO.<br/> If multiple identity providers are configured, the user will choose their desired SAML provider<br/> 
