...
{{% warning  %}}
**Yakkl will stop building and deploying sites on January 1st, 2020**.<br/>
...
In order for Yakkl to deploy your site to Amazon S3, this guide will walk you through:

* Configuring an Amazon S3 bucket to serve your static site.
* Creating an Amazon IAM User to give Yakkl access to your S3 Bucket.
* Configuring your Yakkl site to deploy to your new S3 bucket.
* Pointing your domain to your S3 bucket using Amazon Route 53.

## Create a Bucket

To get started, you must create an S3 bucket for Yakkl to deploy to.
...
* Set the name of your bucket to your website domain _(e.g, yakkl.com)_ , select a region _(e.g, US north)_,  and click `Create`.
...
* Replace `example.com` in the `Resource` field with the name of your bucket _(e.g, yakkl.com)_.
* Click `Save`
...
Amazon AWS uses IAM users to control access to different services. You should setup an IAM user for usage in Yakkl that only has access to your website bucket.
...
* Set the user name to `yakkl` and ensure that `programmatic access` is checked.
* Click `Next`.
...
* Set the `Amazon Resource Name (ARN)` to `arn:aws:s3:::your-bucket-name`. _Ensure you change_ `_your-bucket-name_` _to the name of your bucket_
* Now click `Next`, give your policy a name *(e.g, yakkl-policy) *, and then click `Create Policy`
...
* Before finishing, click the `Download .csv` button. This will download the user information, including the `Access Key ID` and `Secret Access Key` you’ll need to set up deployment in Yakkl.

## Setup S3 in Yakkl

Now that you have an S3 Bucket created with an IAM user that Yakkl can use to access it, we can now set up deployment for your site!

![](/uploads/2018/09/deployment.png)

* Navigate to the `Settings` page in your site on Yakkl, and then click the `Deployment` tab.
* From here, set the `Connect` as `Amazon S3`
* Now you must fill out the connection settings with everything we just set up.
* Enter your new bucket’s name in `Bucket` _(e.g, yakkl.com)_
...
* You can optionally add a subpath to your deployment, in case you want to deploy to a sub-directory* (e.g, yakkl.com/blog)*
* Click `Save Settings`.
...
* Set your domain name to be the same as your bucket (_e.g, yakkl.com_), and then click `Create`.

### Create A Records

You must create an A-record that tells the DNS which S3 bucket to serve. We need to do this for your root domain _(e.g, yakkl.com)_ and your www. subdomain _(e.g, www.yakkl.com)_.
...
Provided everything is set up correctly, your domain should be serving your S3 bucket. From here on, every time you save or publish a page Yakkl will build your site and deploy to S3.
