...
---
You [setup your site](/docs/quickstart/setup-site/ "Setup Your Site") with Yakkl and now you're ready to configure your CMS.  This doc will go over:
...
The folder within your repo to store the uploaded media. If this is set to a directory that doesn't exist, Yakkl will create that directory.

### Front Matter File URL

The path that will **prepend** media files uploaded from Front Matter fields. For example, if this value were set to `/uploads` Yakkl would write out the following image value.
...
This usually has the same value as the `Front Matter File URL`, but in some cases you may need these paths to be different. If this value were set to `/uploads` Yakkl would write out the following in your Markdown body:
...
However, Jekyll supports static assets in your project root.  For this reason, we often see the following File Path settings in Yakkl:
...
When these variables are used in the _Uploads Folder_ setting, Yakkl will create the necessary folders if they don't exist when a file is uploaded.
...
While editing content in the CMS, users can preview their unsaved changes in an ephemeral copy of your full website. In order to do this, you will need to tell Yakkl how to build your site by [configuring the preview environment](/docs/previews/about-previews/).
...
When you sync a new site with Yakkl, all data found in the Front Matter for your pages will be displayed as UI fields.
...
You probably noticed Yakkl committed a .yakkl folder to your repository. This stores all of your sites configuration (FMTs, image settings, etc). If you prefer working in a text file, you can set all of your site's configuration by [editing these files](/docs/settings/config-files/).
